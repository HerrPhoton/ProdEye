...
if __name__ == "__main__":
    ...
    if args.recursive:
        for address, dirs, files in os.walk(args.images_dir):
            for name in files:
                ...
                if args.labels_dir is not None:
                    os.rename(os.path.join(labels_subdir,
                                           Path(name).with_suffix('.txt')),
                              os.path.join(labels_subdir,
                                           Path(new_name).with_suffix('.txt')))
        ...
        for name in os.listdir(args.images_dir):
            ...
            if args.labels_dir is not None:
                os.rename(os.path.join(args.labels_dir,
                                       Path(name).with_suffix('.txt')),
                          os.path.join(args.labels_dir,
                                       Path(new_name).with_suffix('.txt')))
